...
/**
 * Plugin Name: NexoPOS Store
 * Plugin URI: http://nexopos.com
 * Description: NexoPOS Store.
 * Version: 1.0.0
 * Author: Blair Jesyer
 * Text Domain: nexopos-store.
 ...
 */
defined('NEXOPOS_STORE') ? NEXOPOS_STORE : define('NEXOPOS_STORE', 'plugin-textdomain');
...
$vars = [
    'slug' => 'nexopos-store',
    'name' => 'NexoPOS Store',
    'namespace' => 'com.nexopos.store',
    'config' => 'com_nexopos_store',
];
...
add_action('admin_notices', function () use ($vars) {
    if (!class_exists('\Themosis\Foundation\Application')) {
        printf('<div class="notice notice-error"><p>%s</p></div>', __('This plugin requires the Themosis framework in order to work.', NEXOPOS_STORE));
    }
    ...
    if (!current_theme_supports($vars['slug']) && current_user_can('switch_themes')) {
        // printf('<div class="notice notice-warning"><p>%s<strong>%s</strong></p></div>', __('Your application does not handle the following plugin: ', NEXOPOS_STORE), $vars['name']);
    }
});
...
container('action')->add('plugins_loaded', function () use ($vars) {
	 ...
	 */
	load_plugin_textdomain(NEXOPOS_STORE, false, trailingslashit(dirname(plugin_basename(__FILE__))).'languages');
